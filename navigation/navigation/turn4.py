...
CHANGES FROM v3:
  ...
  - RED blob detection added (HSV wraps: H=0-8 OR H=170-180, S>=140, V>=80).
    When red blob depth < 0.5 m → publish Twist(0,0) STOP and enter S_STOP state.
...
RED_STOP_DIST = 0.50    # metres — stop if red blob closer than this
MIN_BLOB_AREA = 400
...
S_STOP               = 'STOP'          # red blob within 0.5 m → hard stop
...
class ArtParkNavNode(Node):
    ...
    def __init__(self):
        ...
        try:
            with open(LOG_PATH, 'w') as f:
                ...
                f.write("=" * 60 + "\n")
                f.write(f"{'ArUco ID':<10} {'Command':<25} {'Timestamp'}\n")
                f.write("-" * 60 + "\n")
        except Exception as ex:
        ...
        try:
            with open(CSV_LOG_PATH, 'w', newline='') as f:
                csv.writer(f).writerow(['id', 'value', 'timestamp'])
        except Exception as ex:
    ...
    def _process(self, frame: np.ndarray) -> Twist:
        ...
        # ── GLOBAL INTERCEPT: RED blob within 0.5 m → hard STOP ──────
        if red is not None and self._state != S_STOP:
    ...
    def _log_marker(self, tid: int, ts: float):
        """
        Called once per ArUco registration.
        Appends a single line to the text log AND a single row to the CSV —
        both immediately on detection so no events are lost if the node crashes.
        The in-memory _marker_log list is kept for any runtime queries.
        """
        cmd = ARUCO_CMD.get(tid, '?')
        self._marker_log.append(MarkerEvent(tid, ts, cmd))
        ts_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(ts))
        self.get_logger().info(f"[ARUCO LOG] ID={tid} cmd={cmd} @{ts_str}")
        ...
        try:
            with open(LOG_PATH, 'a') as f:
                f.write(f"{tid:<10} {cmd:<25} {ts_str}\n")
        except Exception as ex:
        ...
        try:
            with open(CSV_LOG_PATH, 'a', newline='') as f:
                csv.writer(f).writerow([tid, value, ts_str])
        except Exception as ex:
