...
Debug overlay now shows real distance in metres sampled from the
depth frame at each colour blob's centroid, e.g. "GREEN 0.84m"
"""
...
class ColourBlob:
    ...
    area: float
    distance_m: Optional[float] = None   # ← NEW: metres from depth frame, None if unavailable
...
class ArtParkNavNode(Node):
    ...
    def _init_apriltag_detector(self):
        try:
            ...
            self._apriltag_detector = None
            self.get_logger().warn('pupil_apriltags not found — using OpenCV ArUco fallback')
    ...
    # ------------------------------------------------------------------
    # Display timer — only place imshow is called
    # ------------------------------------------------------------------
    ...
    def _process_frame(
        ...
        # 2. Colour blobs — pass depth so centroids can be sampled
    ...
    # ------------------------------------------------------------------
    # Colour blob detection — now samples depth at centroid
    # ------------------------------------------------------------------
    def _detect_blob(
        self,
        hsv:   np.ndarray,
        ...
        depth: Optional[np.ndarray] = None,   # ← NEW parameter
    ) -> Optional[ColourBlob]:
        ...
        if depth is not None:
            dh, dw = depth.shape[:2]
            # Clamp centroid coords to depth frame bounds (may differ in resolution)
            ...
            # Sample a small 5×5 patch around centroid and take the median
            # (single pixel can be NaN in Gazebo depth images)
