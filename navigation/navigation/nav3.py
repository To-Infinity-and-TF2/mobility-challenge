...
class ArtParkNavNode(Node):
    ...
    # ------------------------------------------------------------------
    def _process_frame(self, frame: np.ndarray) -> Twist:
        ...
        if tag is not None:
            ...
            self.get_logger().info(
                f"[AprilTag {tag['id']}] lin={twist.linear.x:.2f}  ang={twist.angular.z:.2f}"
            )
            if debug is not None:
                self._draw_overlay(debug, f"APRILTAG {tag['id']}", twist)
                cv2.imshow("ArtPark Nav [sim]", debug)
            return twist
        ...
        # 3. Decide Twist — use depth if available
        if self.depth_frame is not None:
            twist = self._blobs_to_twist_with_depth(green_blob, orange_blob, h)
        else:
